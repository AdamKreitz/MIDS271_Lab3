...
```{r build and describe data, echo=F, warning=F}
# Create states list
...
```{r average year plot, echo=F, warning=F, message=F, fig.height=3}
...
```{r average year table, echo=F, warning=F}
avg_total_fatalities_rate %>%
...
```{r box plot of total fatality rate by speed limit, echo=F, warning=F, fig.height=2.4}
data %>%
...
```{r total fatality rate per state, echo=F, warning=F, message=F}
data %>%
...
```{r boxplot of total fatality rate by bac, echo=F, warning=F, fig.height=3}
data %>%
...
```{r expanded lm effects, echo=F, warning=F}
...
```{r check multicollinearity, echo=F, warning=F}
...
```{r random effects model, echo=F, warning=F}
...
```{r hausman test, echo=F, warning=F}
...
```{r read monthly miles driven data, echo=F, warning=F, message=F}
monthly_miles <- read_csv("../data/monthly_miles_driven.csv")
```

```{r get population, echo=F, warning=F, message=F}
...
```{r pandemic comparison setup, echo=F, warning=F}
...
```{r pandemic comparison results, echo=F, warning=F}
...
```{r estimates setup, echo=F, warning=F}
log_miles_pc_coef <- fixed_model$coefficients[34] %>% unname()

log_miles_pc_sd <- sqrt(fixed_model$vcov[34,34])
z <- qnorm(.975)
...
```{r estimates results, echo=F, warning=F}
forecast_ci
```
...
```{r serial tests, echo=F, warning=F}
pbgtest(fixed_model)
pdwtest(fixed_model)
```
...
The p-value for the Durbin-Watson test was significantly less than 0.05, which means we reject the null hypothesis and conclude that there is serial correlation. However, the Durbin-Watson test is only limited to singular lag, which makes it a less robust test than the Breusch-Godfrey test. In order to be confident in our assessment, we used the Breusch-Godfrey test, which resulted in a p-value significantly less than .05, which means that we reject the null hypothesis and conclude that there is serial correlation.

```{r heteroskedasticity test, echo=F, warning=F}
