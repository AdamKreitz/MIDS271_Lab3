...
# DEL instead of creating new variable, can just rename year variable
...
data_81_thru_04 <- data %>%
  filter(year_of_observation >= 1981 & year_of_observation <= 2004)

# Fit linear model
lm_model <- lm(total_fatalities ~ factor(year_of_observation), data = data_81_thru_04)
...
exp_lm_model <- lm(total_fatalities ~ factor(year_of_observation) + bac + perSe + sbprim + sbsecon + 
