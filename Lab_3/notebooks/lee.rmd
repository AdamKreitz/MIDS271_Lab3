...
```{r}
expanded_model <- lm(
    total_fatalities_rate ~ year_of_observation + blood_alcohol_limit + per_se_law + seatbelt_law + speed_limit_70_up + graduated_drivers_licenses + population_14_to_24_percent + unemployment_rate_percent + vehicle_miles,
    data = data_df
)
...
state_random_effects_model <- plm(
    total_fatalities_rate ~ year_of_observation + per_se_law + seatbelt_law + speed_limit_70_up + graduated_drivers_licenses + population_14_to_24_percent + unemployment_rate_percent + vehicle_miles,
    model = "random",
    index = "state",
    effect = "individual",
    data = data_df
...
```{r}
current_data <- read_csv("https://fred.stlouisfed.org/graph/fredgraph.csv?bgcolor=%23e1e9f0&chart_type=line&drp=0&fo=open%20sans&graph_bgcolor=%23ffffff&height=450&mode=fred&recession_bars=on&txtcolor=%23444444&ts=12&tts=12&width=1318&nt=0&thu=0&trc=0&show_legend=yes&show_axis_titles=yes&show_tooltip=yes&id=M12MTVUSM227NFWA&scale=left&cosd=1970-12-01&coed=2023-05-01&line_color=%234572a7&link_values=false&line_style=solid&mark_type=none&mw=3&lw=2&ost=-99999&oet=99999&mma=0&fml=a&fq=Monthly&fam=avg&fgst=lin&fgsnd=2020-02-01&line_index=1&transformation=lin&vintage_date=2023-08-03&revision_date=2023-08-03&nd=1970-12-01") %>%
    rename(date = DATE, millions_of_miles = M12MTVUSM227NFWA) %>%
...
```{r}
current_years_df <- current_df %>%
    group_by(year) %>%
    summarise(miles_driven = sum(millions_of_miles))

year2018_df <- current_years_df %>% filter(year == 2018)
year2020_df <- current_years_df %>% filter(year == 2020)
year2021_df <- current_years_df %>% filter(year == 2021)

year2020_df$miles_driven - year2018_df$miles_driven

year2021_df$miles_driven - year2018_df$miles_driven

data.frame(
    year = c("2020 - 2018", "2021 - 2018"),
    `miles driven difference` = c(
        year2020_df$miles_driven - year2018_df$miles_driven,
        year2021_df$miles_driven - year2018_df$miles_driven
    ),
    `percent difference` = c(
        (year2020_df$miles_driven - year2018_df$miles_driven) / year2018_df$miles_driven,
        (year2021_df$miles_driven - year2018_df$miles_driven) / year2018_df$miles_driven
    )
) %>% kable()
```
