...
data$year_of_observation <- data$year
...
data <- data %>%
  rename(total_fatalities_rate = totfatrte,
...
> Our target variable, `total_fatalities_rate`, is defined as the the number of fatalities per 100,000 residents. The dataset is a time-series one that ranges from 1980-2004, tracking traffic fatality data as well as other things such as laws around BAC and speed limits. Additionally, the dataset contains information divided by all 50 states. Each of the columns represents a different variable, whereas each row represents state data from a specific year (with the year being the index.)

> The data is collected, not measured through a survey. This would be the equivalent of census data, as traffic fatalities (really fatalities of any kind) are carefully recorded. Additionally, things such as laws and population are also carefully recorded, which makes up our dataset.
...
```{r Conduct EDA on the dataset}
library(ggplot2)
library(patchwork)

# Scatter plot of total_fatalities_rate vs. vehicmiles
#ggplot(data = data, aes(x = vehicmiles, y = total_fatalities_rate)) +
#  geom_point()

# Box plot of total_fatalities_rate by state
state_box <- ggplot(data = data, aes(x = as.factor(state), y = total_fatalities_rate)) +
  geom_boxplot()

# Bar plot of average total_fatalities_rate by year
fat_by_year = ggplot(data = data, aes(x = as.factor(year), y = total_fatalities_rate)) +
  stat_summary(fun = mean, geom = "bar")

# Density plot of total_fatalities_rate
#ggplot(data = data, aes(x = total_fatalities_rate)) +
#  geom_density()

# Bar plot of total_fatalities_rate by BAC
bac_plot <- ggplot(data = data, aes(x = as.factor(bac), y = total_fatalities_rate)) +
  geom_boxplot()

# Box plot of total_fatalities_rate by Speed Limit
sl_plot <- ggplot(data = data, aes(x = as.factor(speed_limit), y = total_fatalities_rate)) +
  geom_boxplot()

# Box plot of total_fatalities_rate by Seatbelt
seatbelt_plot <- ggplot(data = data, aes(x = as.factor(seatbelt), y = total_fatalities_rate)) +
  geom_boxplot()

# Box plot of total_fatalities_rate by Zero Tolerance
zero_plot <- ggplot(data = data, aes(x = as.factor(zeroTolerance), y = total_fatalities_rate)) +
  geom_boxplot()

# Box plot of minimum age to get a drivers license
minage_plot <- ggplot(data = data, aes(x = as.factor(minAge), y = total_fatalities_rate)) +
  geom_boxplot()

# Box plot of fatalities based on per se laws
perse_plot <- ggplot(data = data, aes(x = as.factor(perSe), y = total_fatalities_rate)) +
  geom_boxplot()

# Scatter plot of unemployment rate vs fatality rate
unem_plot <- ggplot(data = data, aes(x = unem, y = total_fatalities_rate)) +
  geom_point()

# Scatter plot of 14-24 rate vs fatality rate
young_plot <- ggplot(data = data, aes(x = perc14_24, y = total_fatalities_rate)) +
  geom_point()

# Plot overall trends
(state_box/fat_by_year)

# Plot trends by Categorical variable
(bac_plot|sl_plot|minage_plot)/(seatbelt_plot|zero_plot|perse_plot)

# Plot trends by Numerical variable
(unem_plot|young_plot)
```

> So what do we notice in our EDA?

> When looking at the overall trends, we can see that there is quite a bit of variance in terms of fatality rate by state. Additionally, we can see there has been a consistent downtrend in fatality rate over the years that has begun to converge.
> When looking at the categorical trends, we can see that nearly every intervention resulted in a lower fatality rate, whether it be seatbelts, age of driverse license, legal BAC to drive, or speed limit. However, there is a definite possibility between collinearity between these variables.
> When looking at the numerical trends, we see unemployment does not have any kind of substantial correlation to fatality rate, where as the percentage of people between the ages of 14-24 does have a small positive correlation with fatality rate.
> Overall, this EDA gives us a good place to start, and suggests that some of the variables present influence fatality rate, but more needs to be done to determine whether these interventions are ultimately worthwhile or not.
...
```{r Estimate a Linear Regression}
...
# Fit linear model
lm_model <- lm(total_fatalities_rate ~ year_of_observation +as.factor(state), data = data_81_thru_04)

summary(lm_model)

plot(data_81_thru_04$year_of_observation, data_81_thru_04$total_fatalities_rate, col=data_81_thru_04$state) +
...
exp_lm_model <- lm(total_fatalities_rate ~ year_of_observation + bac + perSe + sbprim + sbsecon + 
...
fixed_model <- plm(total_fatalities_rate ~ year_of_observation + bac + perSe + sbprim + sbsecon + 
...
> Main assumption of the fixed effect model is that there is _no_ correlation between residuals and explanatory variables. If the random effects model is acceptable, then it is also more efficient then the fixed effects model because it specifies the error term and serial
correlation correctly. To see if this assumption is met, we run the Hausman Test:

```{r Hausman test}
re_model <- plm(total_fatalities_rate ~ year_of_observation + bac + perSe + sbprim + sbsecon + 
                      sl70plus + gdl + perc14_24 + unem + vehicmilespc, 
                    data = data_81_thru_04_plm,
                    model = "random", 
                    effect = "individual")

phtest(fixed_model, re_model)

```

> We reject the null hypothesis, that random effects are appropriate. The null hypothesis of this test is that Random Effects are appropriate, and thus more efficient than the fixed effect model. This result implies that a Random effects model, in this case, is neither more efficient or consistent.
