...
#ggplot(data = data, aes(x = vehicmiles, y = log_fatility_rate)) +
#  geom_point()

# Box plot of total_fatalities_rate by state
state_box <- ggplot(data = data, aes(x = as.factor(state), y = log_fatility_rate)) +
  geom_boxplot()

# Bar plot of average total_fatalities_rate by year
fat_by_year = ggplot(data = data, aes(x = as.factor(year), y = log_fatility_rate)) +
  stat_summary(fun = mean, geom = "bar")

# Density plot of total_fatalities_rate
#ggplot(data = data, aes(x = log_fatility_rate)) +
#  geom_density()

# Bar plot of total_fatalities_rate by BAC
bac_plot <- ggplot(data = data, aes(x = as.factor(bac), y = log_fatility_rate)) +
  geom_boxplot()

# Box plot of total_fatalities_rate by Speed Limit
sl_plot <- ggplot(data = data, aes(x = as.factor(speed_limit), y = log_fatility_rate)) +
  geom_boxplot()

# Box plot of total_fatalities_rate by Seatbelt
seatbelt_plot <- ggplot(data = data, aes(x = as.factor(seatbelt), y = log_fatility_rate)) +
  geom_boxplot()

# Box plot of total_fatalities_rate by Zero Tolerance
zero_plot <- ggplot(data = data, aes(x = as.factor(zeroTolerance), y = log_fatility_rate)) +
  geom_boxplot()

# Box plot of minimum age to get a drivers license
minage_plot <- ggplot(data = data, aes(x = as.factor(minAge), y = log_fatility_rate)) +
  geom_boxplot()

# Box plot of fatalities based on per se laws
perse_plot <- ggplot(data = data, aes(x = as.factor(perSe), y = log_fatility_rate)) +
  geom_boxplot()

# Scatter plot of unemployment rate vs fatality rate
unem_plot <- ggplot(data = data, aes(x = unem, y = log_fatility_rate)) +
  geom_point()

# Scatter plot of 14-24 rate vs fatality rate
young_plot <- ggplot(data = data, aes(x = perc14_24, y = log_fatility_rate)) +
  geom_point()
...
data %>%
  filter(state %in% sample_n(select(data, c(state)), 16)$state) %>%
  ggplot(aes(year,log_fatility_rate,color=state)) +
...
# Fit linear model
lm_model <- lm(log_fatility_rate ~ year_of_observation, data = data_81_thru_04)

summary(lm_model)

plot(data_81_thru_04$year, data_81_thru_04$log_fatility_rate, col=data_81_thru_04$state) +
...
```{r Hausman test}
re_model <- plm(log_fatility_rate ~ year_of_observation + bac + perSe + sbprim + sbsecon + 
                      sl70plus + gdl + perc14_24 + unem + vehicmilespc, 
                    data = data_81_thru_04_plm,
                    model = "random", 
                    effect = "individual")
